  ...
  try{
    ...
    for (const s of list) {
      ...
      div.onclick = async () => {
        ...
        if (window.addCustomLog) {
          window.addCustomLog(`client send request (${serviceId}, Gas=1, Cost=${costPref}, Delay=${delayPref})`);
        }
        ...
        const targetUrl = `./service-invocation.html?serviceId=${encodeURIComponent(serviceId)}&costPref=${encodeURIComponent(costPref)}&delayPref=${encodeURIComponent(delayPref)}`;
